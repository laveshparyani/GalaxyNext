...
Never commit API keys, passwords, `frappe-bench/sites/*/site_config.json` or database backups.
The bench directory is versioned here, so `.gitignore` excludes site configs, private files and backups; keep it so.
